import Collapsible from "react-collapsible";
import { FlexBody, FlexColumn, FlexContent } from "@components/home/Flex";
import { Navbar, NavbarItem } from "@components/home/Navbar";
import { Menu, MenuItem } from "@components/home/Menu";
import {
  ContentBody,
  ContentItem,
  InfoContainer,
  ContentInfo,
} from "@components/home/Content";
export default function Home() {
  return (
    <FlexBody>
      <FlexColumn>
        <Navbar>
          <NavbarItem />
          <NavbarItem style={{ textAlign: "center" }}>
            <div className="navbarItem" style={{ textAlign: "center" }}>
              <div className="input-field">
                <span className="material-icons dark dark md-light prefix">
                  search
                </span>
                <input type="text" name="pesquisa" placeholder=" Pesquisar " />
              </div>
            </div>
          </NavbarItem>
          <NavbarItem>
            <img src="data:image/png;base64," />
          </NavbarItem>
        </Navbar>
      </FlexColumn>
      <FlexColumn>
        <Menu>
          <MenuItem>
            <span className="material-icons dark dark">home</span>
            <span id="inicio">Inicio</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">person</span>
            <span id="dadosPessoais">Dados pessoais</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">groups</span>
            <span id="turmas">Turmas</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">date_range</span>
            <span id="horarios">Horarios</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">leaderboard</span>
            <span id="desempenho">Desempenho</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">assignment</span>
            <span id="tarefas">Tarefas</span>
          </MenuItem>
          <MenuItem>
            <span className="material-icons dark">inbox</span>
            <span id="noticias">Notícias</span>
          </MenuItem>
        </Menu>
        <FlexContent>
          <ContentItem>
            <div className="slider">
              <ul className="slides">
                <li>
                  <div className="slide-item">
                    <div className="item-title">
                      <h5>Reposição de aula (EDUCAÇÃO FÍSICA) dia 27/05</h5>
                    </div>
                    <div className="item-text">
                      Prezado(a) aluno(a):
                      <br />
                      Olá!
                      <br />
                      Na quinta-feira (dia 27 de Maio), no mesmo horário da aula
                      de Língua Portuguesa, haverá a reposição de aula de
                      Educação Física (repondo a aula ausente do mês de Março).
                      <br />
                      Logo, para este encontro ASSÍNCRONO decidi adiantar um
                      podcast gravado (desta vez, abordando sobre o cérebro), o
                      qual peço-lhes que ouçam com atenção e ANOTEM SUAS DÚVIDAS
                      E COMENTÁRIOS.
                      <br />
                      Em breve, enviarei instruções sobre como enviá-los.
                      <br />
                      Creio que vocês vão gostar! Fiquem à vontade para ouvir
                      quantas vezes desejarem!
                      <br />
                      Grande abraço e até nosso próximo encontro!
                      <br />
                    </div>
                  </div>
                </li>
                <li>
                  <div className="slide-item">
                    <div className="item-title">
                      <h5>Reposição de aula (EDUCAÇÃO FÍSICA) dia 27/05</h5>
                    </div>
                    <div className="item-text">
                      Prezado(a) aluno(a):
                      <br />
                      Olá!
                      <br />
                      Na quinta-feira (dia 27 de Maio), no mesmo horário da aula
                      de Língua Portuguesa, haverá a reposição de aula de
                      Educação Física (repondo a aula ausente do mês de Março).
                      <br />
                      Logo, para este encontro ASSÍNCRONO decidi adiantar um
                      podcast gravado (desta vez, abordando sobre o cérebro), o
                      qual peço-lhes que ouçam com atenção e ANOTEM SUAS DÚVIDAS
                      E COMENTÁRIOS.
                      <br />
                      Em breve, enviarei instruções sobre como enviá-los.
                      <br />
                      Creio que vocês vão gostar! Fiquem à vontade para ouvir
                      quantas vezes desejarem!
                      <br />
                      Grande abraço e até nosso próximo encontro!
                      <br />
                      reposição de aula de Educação Física (repondo a aula
                      ausente do mês de Março).
                      <br />
                      Logo, para este encontro ASSÍNCRONO decidi adiantar um
                      podcast gravado (desta vez, abordando sobre o cérebro), o
                      qual peço-lhes que ouçam com atenção e ANOTEM SUAS DÚVIDAS
                      E COMENTÁRIOS.
                      <br />
                      Em breve, enviarei instruções sobre como enviá-los.
                      <br />
                      Creio que vocês vão gostar! Fiquem à vontade para ouvir
                      quantas vezes desejarem!
                      <br />
                      Grande abraço e até nosso próximo encontro!
                      <br />
                    </div>
                  </div>
                </li>
                <ul className="indicators">
                  <div className="btn-slides">
                    <div className="nextbtn">
                      <span className="material-icons dark md-48 left">
                        chevron_left
                      </span>
                    </div>
                    <div className="nextbtn">
                      <span className="material-icons dark md-48 right">
                        chevron_right
                      </span>
                    </div>
                  </div>
                </ul>
              </ul>
            </div>
          </ContentItem>
          <ContentItem>
            <Collapsible
              trigger={
                <ContentBody>
                  <span className="material-icons">label</span>
                  <span>Artes II</span>
                </ContentBody>
              }
              easing="ease-in-out"
            >
              <ContentInfo>
                <InfoContainer>
                  <span>Horário: </span>
                  <span>5T2</span>
                </InfoContainer>
                <InfoContainer>
                  <span>Código: </span>
                  <span>ART027602 </span>
                </InfoContainer>
                <InfoContainer>
                  <span className="material-icons dark">leaderboard</span>
                  <span>Ver notas</span>
                  <span className="material-icons dark">assignment</span>
                  <span>Tarefas</span>
                  <span className="material-icons dark">login</span>
                  <span>Entrar</span>
                </InfoContainer>
              </ContentInfo>
            </Collapsible>
          </ContentItem>
        </FlexContent>
      </FlexColumn>
    </FlexBody>
  );
}
